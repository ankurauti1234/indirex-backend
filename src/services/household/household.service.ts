...
async updatePreassignedContact(
  ...
  async assignMembersManually(
    ...
    if (household.members && household.members.length > 0) {
      throw new Error(
        `HHID "${normalizedHhid}" already has members assigned. Cannot overwrite existing assignment.`
      );
    }
    ...
    const memberEntities = members.map((m) => {
      ...
      if (m.dob) {
        dob = new Date(m.dob);
        if (isNaN(dob.getTime())) throw new Error(`Invalid DOB for ${m.memberCode}`);
      } else {
        dob = new Date(today);
  ...
  async getPreregisteredEmails(search?: string): Promise<string[]> {
    ...
    const rows = await qb.limit(30).getRawMany<{ email: string }>();
